...
exports.getUnseenUsersMessagesNumber = async(req, res) => {
    try {
        ...
        const msgs = users.filter(user => new Date(user.lastSeen).getTime() > new Date(user.lastSeenAdmin).getTime());
...
exports.chatSocket = async(io, socket) => {
    ...
    /////////////////////
    // socket.on("heartbeat", () => {
    //   setTimeout(() => socket.emit("heartbeat"), 10000);
    // });
    /////////////////////
...
//         if(!online[socket.id]) online[socket.id] = data.role == 1? "admin": data.room;
//     });

//     socket.on("sendMessageAdmin", async(message) => {
//         io.in(message.room).emit("receiveMessage", message);
//         await Messages({...message}).save();
//     });
//     /////////////////////
//     socket.on("sendMessage", async(message) => {
...
//         const onlineUsers = [];
//         Object.keys(sockets).forEach(sock => {
//             if(online[sock]) onlineUsers.push(online[sock]);
//             else delete online[sock];
//         });
...
//         await Users.updateOne({_id: userId}, {lastSeenAdmin: new Date()});
//     });
//     socket.on("deleteMsg", async(code) => {
//         io.emit("deleteMsg", code);
//         await Messages.deleteOne({code});
//     })
// }