...
exports.getUserTaskAndSolution = async(req, res) => {
    try {
        let task = false;
        let solution = false;
        const tasks = await Tasks.find({lessonId: req.body.lessonId}).count();
        if(tasks > 0) task = true;
        ////////////////////////////
        const solutions = await tasksSolutions.find(req.body).count();
        if(solutions > 0) solution = true;
        console.log(task, solution)
        res.json({solution, task});
    } catch(err) {
        console.log(err);
        res.json({err});
    }
...
exports.removeTask = async(req, res) => {
    try {
        ...
        if(deleted.n == 1) {
            ...
            res.json({done: true});
        } else throw("مشكلة في حذف الواجب");
    } catch(err) {
        console.log(err);
        res.json({err});
    }
